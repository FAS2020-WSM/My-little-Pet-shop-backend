...
class PetController
{
    ...
    public function add(Request $request, EntityManagerInterface $em): JsonResponse
    {
        ...
        $name = $data['name'] ?? null;
        $type = $data['type'] ?? null;
        $photoUrls = $data['photo_url'] ?? null;

        if(empty($name) || empty($type)){
            throw new NotFoundHttpException('Expecting mandatory parameters!');
        }

        //$x = $em -> getRepository(Pet::class) -> findOneBy(['id' => $name]); = Possible Delete / Update
    ...
    public function get($id): JsonResponse
    {
        ...
        // If not found
        if (!$pet) {
            return new JsonResponse(['Message' => 'Pet not found'], Response::HTTP_UNPROCESSABLE_ENTITY);
        }
    ...
    public function update($id, Request $request): JsonResponse
    {
        ...
        empty($data['photo_url']) ? true : $pet->setPhotoUrl($data['photo_url']);
